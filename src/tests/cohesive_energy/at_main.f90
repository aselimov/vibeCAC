...
!   ------------------------------------------------------------
    ...
    command='read_data Cu_at.restart'
    ...
    if (rank==root) then 
        if (abs(pe_check - pe_real) < 0.01) then
            print *, '[OK] - Potential energy calculation success for atomistic model'
            test_results(1) = .true.
        else
            print *, '[FAILED] - Potential energy calculation failure for atomistic model. CAC = ', pe_check, &
                     ' reference = ',pe_real
        end if

        if (abs(fnorm_check - fnorm_real) < 1d-8) then
            print *, '[OK] - Force norm calculation success for atomistic model'
            test_results(2) = .true.
        else
            print *, '[FAILED] - Force norm calculation failure for atomistic model. CAC = ', fnorm_check, &
